...
public class EJSDateUtils {
     ...
     * 从字符串解析成时间、日期
     */
    public static Date parseDateForNull(String dateStr, DateFormatType dateFormatType) {
        Date date;
        try{
            date = (Date) doOperation(dateStr, dateFormatType.getValue());
        }catch (Exception e){
            date = null;
        }
        return date;
     ...
     * @return
     */
    public static boolean isNew(Date curDate,Date oriDate){
        if(oriDate == null){
            return true;
        }
        ...
        return curDate.getTime() > oriDate.getTime();
    }
}
