    ...
    private void addGiftByActivity(Order order) {
        ...
        for(Map.Entry<Activity, List<OrderItem>> entry : activityToOrderItemMap.entrySet()) {
            ...
            for(OrderItem orderItem : orderItemList) {
                buyCount += orderItem.getRealBuyCount();
            }
            for(ActivityItem activityItem : activity.getActivityItems()) {
                ...
                int giftAmount = activityItem.getAmount();
                if(ActivityType.PRODUCT.equals(activity.getType())) {
                    giftAmount = buyCount * giftAmount;
                }
                giftAmountMap.put(productId, productAmount + giftAmount);
            }
        }
