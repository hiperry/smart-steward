...
public class OrderExcelUtil {
    ...
    public static int createExcelOrderTitle(Sheet sheet) {
        ...
        PoiUtil.createCell(row, cellIndex++, "平台优惠金额");
        PoiUtil.createCell(row, cellIndex++, "店铺优惠金额");
        ...
        PoiUtil.createCell(row, cellIndex++, "支付宝账号");
...
//        sheet.addMergedRegion(new CellRangeAddress(0, 0, orderCellIndex + 1, cellIndex - 1));  //合并商品条目标题的单元格
    ...
    public static int renderOrderVoExcel(Row row, int startCellIndex, OrderVo order) {
        ...
        PoiUtil.createCell(row, startCellIndex++, order.getOutActualFee() != null ? order.getOutActualFee() :"0.00");
        PoiUtil.createCell(row, startCellIndex++, order.getSharedDiscountFee() != null ? order.getSharedDiscountFee() : "0.00");
        PoiUtil.createCell(row, startCellIndex++, order.getSelfSharedDiscountFee() != null ? order.getSelfSharedDiscountFee().toString() :"0.00");
        PoiUtil.createCell(row, startCellIndex++, order.getPostFee() != null ? order.getPostFee() : "0.00");
        PoiUtil.createCell(row, startCellIndex++, order.getGoodsFee() != null ? order.getGoodsFee() : "0.00");
        ...
        PoiUtil.createCell(row, startCellIndex++, order.getBuyerAlipayNo()!= null ? order.getBuyerAlipayNo() : "");
    ...
    public static int renderOrderItemVoExcel(Row row, int startCellIndex, OrderItemVo orderItemVo) {
        ...
        PoiUtil.createCell(row, startCellIndex++, orderItemVo.getSharedDiscountFee() != null ? orderItemVo.getSharedDiscountFee().toString() : "0.00");
        PoiUtil.createCell(row, startCellIndex++, orderItemVo.getSelfSharedDiscountFee() != null ? orderItemVo.getSelfSharedDiscountFee().toString() : "0.00");
        ...
        PoiUtil.createCell(row, startCellIndex++, orderItemVo.getGoodsFee() != null ? orderItemVo.getGoodsFee().toString() : "0.00");
   ...
   public static void collectNum(int rowIndex,Workbook workbook,Sheet sheet,int size,int productNum){
    ...
    public static void judgeLeadInOrder(ShopService shopService, ProductService productService, Row row, int rowNum, List<Order> orders) {
        ...
        leadInExceptionMess(order.getPlatformType(), String.format("导入失败，检测到第%s行订单的平台类型匹配不上系统的平台类型，请查看Excel平台类型是否正确",rowNum));
        ...
        //判断价格是否为数值类型方法
        judgeIsNumber(row,4,String.format("导入失败，检测到第%s行订单价格为空或者不是数值类型",rowNum));
        ...
        //判断订货数量是否为数值类型方法
        judgeIsNumber(row,5,String.format("导入失败，检测到第%s行订单没有订货数量为空或者不是数值类型",rowNum));
        orderItem.setBuyCount(leadInExceptionMess(PoiUtil.getIntegerCellValue(row, 5), String.format("导入失败，检测到第%s行订单没有订货数量", rowNum)));
        //判断分摊邮费是否为数值类型方法
        judgeIsNumber(row,6,String.format("导入失败，检测到第%s行订单分摊邮费为空或者不是数值类型",rowNum));
        ...
        invoice.setShippingComp(StringUtils.isBlank(PoiUtil.getStringCellValue(row, 18))?null:DeliveryType.enumValueOf(PoiUtil.getStringCellValue(row, 18).trim())+"");
        leadInExceptionMess(invoice.getShippingComp(), String.format("导入失败，检测到第%s行订单的快递公司没被收录在系统里或者为空", rowNum));
        ...
        leadInExceptionMess(buyDate, String.format("导入失败，检测到第%s行订单没有下单时间或者格式错误,时间格式为2014-10-10 10:00:00", rowNum));
    ...
    public static void judgeIsNumber(Row row,int index,String mess){
        if(!PoiUtil.judgeCellType(row,index)){
            throw new StewardBusinessException(mess);
        }
    }
}
