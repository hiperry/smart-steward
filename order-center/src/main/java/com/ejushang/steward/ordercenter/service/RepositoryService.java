...
public class RepositoryService {
    ...
    public void deleteById(String ids) {
        ...
        for (int i : id) {
            ...
            if (storageList.size() > 0) {
                ...
                // 删除管理人员
                Query query = generalDAO.getSession().createQuery("delete from RepositoryCharger where repositoryId=:repoId");
                query.setInteger("repoId", i);
                query.executeUpdate();
    ...
    public void add(Repository repository, Integer[] chargerIds) {
        ...
        repository.setAreaId(StringUtils.isNotBlank(repository.getAreaId()) ? repository.getAreaId() : null);
    ...
    public void save(Repository repository, Integer[] chargerIds) {
        ...
        if (StringUtils.isNotBlank(repository.getName())) {
            Repository repositorieTmp = findByName(repository.getName());
            if (repositorieTmp != null && !repositorieTmp.getId().equals(repository.getId())) {
                throw new StewardBusinessException("已经有该仓库名称");
            }
        }
        if (StringUtils.isNotBlank(repository.getCode())) {
            Repository repositorieTmp = findByCode(repository.getCode());
            if (repositorieTmp != null && !repositorieTmp.getId().equals(repository.getId())) {
        ...
        Repository oldRepository = generalDAO.get(Repository.class, repository.getId());
        oldRepository.setName(repository.getName());
        oldRepository.setCode(repository.getCode());
        oldRepository.setProvinceId(repository.getProvinceId());
        oldRepository.setCityId(repository.getCityId());
        oldRepository.setAreaId(StringUtils.isNotBlank(repository.getAreaId()) ? repository.getAreaId() : null);
        oldRepository.setAddress(repository.getAddress());
        oldRepository.setChargeMobile(repository.getChargeMobile());
        oldRepository.setChargePhone(repository.getChargePhone());
        oldRepository.setCustid(repository.getCustid());
        oldRepository.setShippingComp(repository.getShippingComp());
        oldRepository.setUpdateTime(new Date());
        oldRepository.setOperatorId(SessionUtils.getEmployee().getId());
     ...
     * 判断用户是否是仓库管理员
     *
     * @param chargerId    用户id
     * @param repositoryId 仓库id
     * @return 如果是该仓库管理员返回true，否则返回false
     */
    @Transactional(readOnly = true)
    public Boolean isRepositoryCharger(Integer chargerId, Integer repositoryId) {
        return generalDAO.count(new Search(RepositoryCharger.class)
                .addFilterEqual("chargerId", chargerId)
                .addFilterEqual("repositoryId", repositoryId)) > 0;
