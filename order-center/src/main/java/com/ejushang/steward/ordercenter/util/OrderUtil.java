...
//            map.remove("orderStatus");}
        ...
        //发货管理的订单不能是退货单
        if (repoIdList != null) {
            if (StringUtils.isBlank(map.get("invoiceInvalid"))) {
...
//======================================================================================================================================
     ...
     * @return
     */
    public static String getExchangeGoodsInfo(OrderItem orderItem, GeneralDAO generalDAO) {
        if (!orderItem.getExchanged() || orderItem.getExchangeOrderItemId() == null || generalDAO == null) {
            return null;
        }
        ...
        String exchangeOrderItemProductName = exchangeOrderItem.getProductName();
        if (exchangeOrderItemProductName == null) {
            return null;
        }
        return String.format("被售前换货,原产品名称(%s)", exchangeOrderItemProductName);
    ...
    public static OrderItemVo getOrderItemVo(OrderItemVo orderItemVo, OrderItem orderItem, OrderType orderType, GeneralDAO generalDAO, Boolean isAble) {
        ...
        orderItemVo.setExchangedGoods(getExchangeGoodsInfo(orderItem, generalDAO));
     ...
     */
    public static OrderVo getOrderVo(Order order, List<OrderVo> orderVos, List<OrderItem> orderItems, Boolean type, PaymentService paymentService) {
        ...
        orderVo.setBuyerAlipayNo(order.getBuyerAlipayNo());
        orderVo.setSelfSharedDiscountFee(order.getSelfSharedDiscountFee());
...
//        orderVo.setBuyerAlipayNo(order.getBuyerAlipayNo());    支付宝账号（暂时不显示）
        ...
        if (orderItemsSize > 0) {
            ...
            Double maxMoney = 0d;
            int index = 0;
            Integer[] orderItemIds = new Integer[orderItems.size()];
            ...
            orderVo.setOrderItemIds(orderItemIds);
        }
...
//         }
    ...
    public static void main(String[] aa) {
        String a = "asdfadsfafas";
        char[] c = a.toCharArray();

        for (int i = 0; i < c.length; i++) {
            String result = c[i] + "出现的次数";
            int num = 0;
