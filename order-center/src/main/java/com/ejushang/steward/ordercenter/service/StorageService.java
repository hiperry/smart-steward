...
//            List<Repository> repositories = generalDAO.search(new Search(Repository.class).addFilterEqual("chargePersonId", employee.getId()));
    ...
    public String batchUpdate(InputStream is) throws IOException {
        ...
        for (Row row : sheet) {
            ...
            try {
                allotProductStorage2Platform(product.getOuterProductNo(), product.getSku(), true);
...
//        storageVO.getRepository().setName(PoiUtil.getStringCellValue(row, StorageExcelCellIndex.REPOSITORY_NAME.index()));
        return storageVO;
    }
...
//        String repositoryName = PoiUtil.getStringCellValue(row, StorageExcelCellIndex.REPOSITORY_NAME.index());
...
//            throw new StewardBusinessException(String.format("仓库[%1$s]下没有编码[%2$s]的产品", repositoryName, sku));
    ...
    public Storage findByProductId(Integer productId) {
        ...
        return (Storage) generalDAO.searchUnique(search);  //业务上暂时定为一个产品对应一个仓库，取第一条数据
    }
...
//                throw new StewardBusinessException(String.format("sku为[%s]的产品[%s]库存不足，不能出库", product.getSku(), product.getName()));
            ...
            Product product = storage.getProduct();
            try {
                allotProductStorage2Platform(product.getOuterProductNo(), product.getSku(), syncAmount);
            } catch (ApiInvokeException e) {
                log.info(e.getMessage(), e);
                throw new StewardBusinessException(e.getMessage(), e);
            }
    ...
    public Storage storageReduce(Integer productId, Integer num, InOutStockType outStockType, Integer orderId, String desc, boolean syncAmount) {
        Storage storage = findStorageByProductIdWithDeletedProduct(productId);
     ...
     * @return
     */
    private Storage findStorageByProductIdWithDeletedProduct(Integer productId) {
        return (Storage) generalDAO.searchUnique(new Search(Storage.class).addFilterEqual("productId", productId));
    }
     ...
     * @param syncAmount 是否同步库存到京东/天猫等外部平台
     */
    private void allotProductStorage2Platform(String productNo, String sku, boolean syncAmount) throws ApiInvokeException {
        for (ProductInvoke invoke : productInvokes) {
            ProductPlatform productPlatform = productPlatformService.findBuProductSkuAndPlatformType(sku, invoke.getType());
            if (productPlatform == null || !productPlatform.getSynStatus()) {
                continue;
            }
            if (syncAmount) {
                invoke.updateProductPlatformStorage(productNo, sku, productPlatform);
            }
