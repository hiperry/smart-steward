...
public class OrderItem implements EntityClass<Integer>, CalculableOrderItem {
    ...
    @javax.persistence.Column(name = "self_shared_discount_fee")
...
//    }
    ...
    public Money calculateDiscountPrice() {
        return calculateOrderItemFee().divide(getRealBuyCount());
    }
    ...
    public Money calculateOrderItemFee() {
        return getPrice().multiply(getRealBuyCount()).subtract(getDiscountFee());
    }
    ...
    public Money getUserActualPrice() {
        ...
        return getDiscountPrice().multiply(getRealBuyCount()).subtract(getSharedDiscountFee()).subtract(getSelfSharedDiscountFee());
    ...
    @Override
    public void setRealBuyCount(Integer realBuyCount) {
    }

    /**
     * 该字段与buyCount的区别是,如果发生了售前换货,该字段可以取得真实的售前换货之前的订购数量.
     * 计算金额会用到该字段
     * @return
     */
    @Transient
    ...
    public Integer getRealBuyCount() {
        ...
        return exchangeOrderItem.getBuyCount();
    }
    ...
    public OrderItem copyForSplit() {
        ...
        copiedOrderItem.setSelfSharedDiscountFee(getSelfSharedDiscountFee());
