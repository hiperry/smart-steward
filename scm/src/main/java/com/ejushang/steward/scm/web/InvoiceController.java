...
public class InvoiceController {
    ...
    public JsonResult updateOrderShipping(Integer orderIds[], String shippingComp, String intNo, String isCover) {
         if(log.isInfoEnabled()){
             log.info(String.format("联想物流编号，订单ID：%s,物流公司:%s,是否覆盖:%s,基数:%s",
     ...
     * @throws java.io.IOException
     */
    @RequestMapping("/SFShipping/update")
    @ResponseBody
    @OperationLog("联想顺丰物流编号")
    public JsonResult updateOrderShipping(Integer orderIds[], String shippingComp, SfConstant.SF_EXPRESS_TYPE expressType) {
        if (log.isInfoEnabled()) {
            log.info(String.format("联想物流编号，订单ID：%s,物流公司:%s,顺丰类型:%s",
                    Arrays.toString(orderIds), shippingComp, expressType));
        }
       String result= logisticsPrintInfoService.updateOrderSFShipping(orderIds, shippingComp, expressType);
        BusinessLogUtil.bindBusinessLog("联想物流编号，订单ID：%s,物流公司:%s,顺丰类型:%s",
                Arrays.toString(orderIds), shippingComp, expressType);
        return new JsonResult(true, String.format("操作成功%s",(StringUtils.isBlank(result)?"":result)));
    }
    ...
    @ResponseBody
    public JsonResult listInvoiceOrderItem(Integer orderId, Boolean invalid) {
        if (invalid == null) {
            invalid = false;
        }
        return new JsonResult(true, "操作成功").addList(invoiceService.listInvoiceOrderItemByOrderId(orderId, invalid));
    }
