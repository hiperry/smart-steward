...
import com.ejushang.steward.ordercenter.domain.OrderPage;
...
public class OrderController extends BaseController {
    ...
    JsonResult findOrderDetails(HttpServletRequest request) throws ParseException {
        ...
        OrderPage orderPage=new OrderPage(page.getPageNo(),page.getPageSize());
        orderService.findOrderDetails(map,orderPage,true);
        return new JsonResult(true).addObject(orderPage);
    }
    ...
    JsonResult deleteByOrderItemById(Integer id) {
        ...
        return new JsonResult(true, orderItem.getOrderId() == null ? "" : String.valueOf(orderItem.getOrderId()));
    }
    ...
    @ResponseBody
    public JsonResult exchangeGoods(Integer oldOrderItemId, String exchangeDetails) {
        List exchangeDetailsList;
        try {
            exchangeDetailsList = JsonUtil.jsonToList(exchangeDetails, Map.class);
            orderService.exchangeGoods(oldOrderItemId, exchangeDetailsList);
        } catch (IOException e) {
            log.error("", e);
            throw new StewardBusinessException("解析售前换货请求参数的时候发生错误,请联系管理员");
        }

        BusinessLogUtil.bindBusinessLog("换货前订单项id[%d],换货后的json[%s]",oldOrderItemId,exchangeDetails);

        return new JsonResult(true, "售前换货操作成功");
    }
