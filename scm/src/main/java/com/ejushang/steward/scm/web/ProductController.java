...
public class ProductController extends BaseController {
    ...
    public JsonResult save(@ModelAttribute("id") Product product, Integer repositoryId, Integer repositoryNum, Integer id, Integer[] platformIdArray, String[] marketPriceArray,
        ...
        for (int i = 0; i < platformIdArray.length; i++) {
             ...
             * 处理数组，设值进去
             */
            ProductPlatform productPlatform = generateProductPlatform(platformIdArray[i], marketPriceArray[i], importPriceArray[i], storageNumArray[i], platformUrlArray[i], storagePercentRealArray[i]);
            if (logger.isInfoEnabled()) {
                logger.info("需要保存的productPlatform：" + productPlatform);
            }
            /**
             * 布尔字段的校验
             */
            productPlatform.setPutaway(isPutawayArray[i]);
            productPlatform.setSynStatus(synStatusArray[i]);
            productPlatformList.add(productPlatform);
        }
        productService.save(product, productPlatformList, repositoryId, repositoryNum);
    ...
    private ProductPlatform generateProductPlatform(Integer platformId, String amount, String amount1, Integer storageNum, String platformUrl, Integer storagePercentReal) {
        ProductPlatform productPlatform = new ProductPlatform();
        productPlatform.setPlatformId(platformId);
        productPlatform.setPrice(Money.valueOf(amount));
        productPlatform.setDiscountPrice(Money.valueOf(amount1));
        productPlatform.setStorageNum(storageNum);
        productPlatform.setPlatformUrl(platformUrl);
        productPlatform.setStoragePercent(storagePercentReal);
    ...
    public JsonResult update(@ModelAttribute("id") Product product, Integer repositoryId, Integer repositoryNum,
        ...
        for (int i = 0; i < platformIdArray.length; i++) {
            ProductPlatform productPlatform = generateProductPlatform(platformIdArray[i], marketPriceArray[i], importPriceArray[i], storageNumArray[i], platformUrlArray[i], storagePercentRealArray[i]);
            productPlatform.setId(productPlatformIdArray[i]);
            productPlatform.setPutaway(isPutawayArray[i]);
            productPlatform.setSynStatus(synStatusArray[i]);
            productPlatformList.add(productPlatform);
        }
    ...
    public JsonResult findProducAddOrder(HttpServletRequest request, String searchType, String searchValue, String platform) {
        ...
        for (Product product : productList) {
            //数据的录入
            Map<String, Object> productVo = extractData(product);
     ...
     * @return
     */
    private Map<String, Object> extractData(Product product) {
    ...
    private Map<String, Object> extractPlatformData(Integer productId) {
        Map<String, Object> data = new HashMap<String, Object>();
        List<ProductPlatform> productPlatforms = productPlatformService.listByProductId(productId);
        for (ProductPlatform productPlatform : productPlatforms) {
            if (logger.isInfoEnabled()) {
                logger.info("商品对应的商品-平台信息：productPlatform" + productPlatform);
            }
            if (productPlatform == null) {
                continue;
            }
            copyPlatformProperties(data, productPlatform);
        }
        return data;
    }
     ...
     */
    private void copyPlatformProperties(Map<String, Object> data, ProductPlatform productPlatform) {
        Platform platform = productPlatform.getPlatform();
        String platformType = platform.getType();
        data.put(platformType + "PlatformId", platform.getId());
        data.put(platformType + "PlatformName", platform.getName());
        data.put(platformType + "MarketPrice", productPlatform.getPrice());
        data.put(platformType + "ImportPrice", productPlatform.getDiscountPrice());
        data.put(platformType + "IsPutaway", productPlatform.getPutaway());
        data.put(platformType + "SynStatus", productPlatform.getSynStatus());
        data.put(platformType + "StorageNum", productPlatform.getStorageNum());
        data.put(platformType + "StoragePercentReal", productPlatform.getStoragePercent());
        data.put(platformType + "PlatformUrl", productPlatform.getPlatformUrl());
        data.put(platformType + "ProductPlatformId", productPlatform.getId());
    }
...
//        data.put("importPrice", product.getImportPrice());
    ...
    private Map<String, Object> putMap(String header, String width, String dataIndex) {
        Map<String, Object> headMap = new HashMap<String, Object>();
        headMap.put("header", header);
        headMap.put("width", width);
    ...
    private void setHeadValue(List<Map<String, Object>> head) {
        head.add(this.putMap("产品ID", "60", "id"));
        head.add(this.putMap("产品品牌", "60", "brandName"));
        head.add(this.putMap("产品名称", "100", "name"));
        head.add(this.putMap("产品编号", "100", "productNo"));
        head.add(this.putMap("sku", "100", "sku"));
        head.add(this.putMap("外部平台产品编码", "100", "outerProductNo"));
        head.add(this.putMap("产品分类", "80", "prodCategoryName"));
        head.add(this.putMap("产品描述", "100", "description"));
        head.add(this.putMap("销售价", "60", "importPrice"));
        head.add(this.putMap("市场价", "60", "marketPrice"));
        head.add(this.putMap("最低价", "60", "minimumPrice"));
        head.add(this.putMap("颜色", "80", "color"));
        head.add(this.putMap("重量", "60", "weight"));
        head.add(this.putMap("尺寸", "100", "boxSize"));
        head.add(this.putMap("规格", "100", "speci"));
        head.add(this.putMap("产地", "100", "orgin"));
        head.add(this.putMap("产品类型", "100", "style"));
        head.add(this.putMap("库位", "100", "location"));
        head.add(this.putMap("总库存", "100", "repositoryNum"));
        head.add(this.putMap("仓库名", "100", "repositoryName"));
    }
    ...
    @ResponseBody
    public JsonResult saveProductPlatform(ProductPlatform productPlatform) {
        if (logger.isInfoEnabled()) {
    ...
    @ResponseBody
    public JsonResult updateProductPlatform(ProductPlatform productPlatform) {
        if (logger.isInfoEnabled()) {
            logger.info("修改产品-平台  productPlatform：" + productPlatform);
        }
    ...
    @ResponseBody
    public JsonResult findProductPlatform(ProductPlatform productPlatform) {
        if (logger.isInfoEnabled()) {
            logger.info("修改产品-平台  productPlatform：" + productPlatform);
        }
        List<ProductPlatform> productPlatformList = productService.findProductPlatform(productPlatform);
        return new JsonResult(true).addList(productPlatformList);
    }
    ...
    @ResponseBody
    public JsonResult deleteProductPlatform(ProductPlatform productPlatform) {
        if (logger.isInfoEnabled()) {
    ...
    public JsonResult detail(Integer id) {
        ...
        for (Platform platform : platforms) {
            ...
            ProductPlatform productPlatform = productService.getProductPlatform(id, platform.getId());
            if (productPlatform == null) {
                productPlatform = new ProductPlatform();
                productPlatform.setPlatformId(platform.getId());
                productPlatform.setPlatform(platform);
                productPlatform.setProdId(pro.getId());
                productPlatform.setProduct(pro);
            }
            copyPlatformProperties(platformVo, productPlatform);
            platformVos.add(platformVo.values());
        }
//        List<ProductPlatform> productPlatforms = productPlatformService.listByProductId(id);
//        for (ProductPlatform productPlatform : productPlatforms) {
//            Map<String, Object> platformVo = new LinkedHashMap<String, Object>();
//            copyPlatformProperties(platformVo, productPlatform);
//            platformVos.add(platformVo.values());
//        }
        ...
        return new JsonResult(true, "操作成功").addObject(res);
    }
}
