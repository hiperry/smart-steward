...
Ext.define('Supplier.controller.Order', {
    ...
    views: ['Order', 'Search', 'List', 'Item', 'Batch', 'AutoNumber', 'SFWin'],
...
//                    if (!com.checkGridSel('OrderList', '请至少选择一项订单'))  return;
                    ...
                    var shippingComps = Espide.Common.getGridSels('OrderList', 'shippingComp');

                    //判断是否全部都是顺丰快递
                    function isArrAllEqual(arr) {
                        var len = arr.length,
                            first = 'shunfeng';
                        for (var i = 1; i < len; i++) {
                            if (first != arr[i]) {
                    ...
                    if (!isArrAllEqual(shippingComps)) {
                        Espide.Common.showGridSelErr('你选择订单中包含不是顺丰快递！');
                        return false;
                    }

                    //如果"顺丰联想单号"视图已绘制就返回
                    if (Ext.getCmp('SFWin')) return;

                    Ext.widget('SFWin').show();
                }
            },
            //联想顺丰订单号
            '#SFWcomfirm': {
                click: function (btn) {
                    var data = {}, com = Espide.Common, orderIds = Espide.Common.getGridSelsId('OrderList');
                    data.orderIds = orderIds.join(',');
                    var newData = Ext.Object.merge(btn.up('form').getValues(), data);

                    com.commonMsg({
                        msg: '你确定要联想单号吗，修改订单后不可复原!',
                        fn: com.doAction({
                            //url: '/invoice/updateShipping',
                            url: 'invoice/SFShipping/update',
                            params: newData,
        ...
        parent.getLogisticsDeign(delivery, function (data, width, height) { // 获取物流单设计代码
            ...
            parent.getOrderPrintInfo(ids.join(), function (data) { // 获取物流信息列表
                ...
                if (dataList.length > 0) {
                    ...
                    //LODOP.PRINT_INITA(0, 0, 1000, 1500, "初始化打印控件");
                    for (var i = 0; i < dataList.length; i++) {
                        LODOP.NewPage();
                        printHtml = parent.replaceAllHtml(designCode, dataList[i]);
                        //防止地址换行符
                        eval(printHtml.replace(/[\r\n]/g,""));
...
//            html = html.replace(new RegExp("收货人联系方式", "g"), (data.invoice.receiver.receiverPhone || '') + ' ' + (data.invoice.receiver.receiverPhone || ''));
        ...
        if (html.indexOf("月结账号号码") > 0) {
            html = html.replace(new RegExp("月结账号号码", "g"), data.custid);
        }

        if (html.indexOf("原寄地网点代码") > 0) {
            html = html.replace(new RegExp("原寄地网点代码", "g"), data.originCode);
        }
        if (html.indexOf("目的地城市代码") > 0) {
            html = html.replace(new RegExp("目的地城市代码", "g"), data.destCode);
        }

        if (html.indexOf("母单号") > 0) {
            html = html.replace(new RegExp("母单号", "g"), '母单号：' + data.shippingNo);
        }

        if (html.indexOf("标记快递类型") > 0) {
            var typeD = data.sfExpressType;
            switch (typeD) {
                case 'BIAO_ZHUN_KUAI_DI':
                    html = html.replace(new RegExp("标记快递类型", "g"), '');
                    break;
                case 'SHUN_FENG_TE_HUI':
                    html = html.replace(new RegExp("标记快递类型", "g"), '');
                    break;
                case 'DIAN_SHANG_TE_HUI':
                    html = html.replace(new RegExp("标记快递类型", "g"), 'E');
                    break;
                case 'DIAN_SHANG_SU_PEI':
                    html = html.replace(new RegExp("标记快递类型", "g"), '');
                    break;
            }
        }
        if (html.indexOf("快递项目") > 0) {
            var type = data.sfExpressType;
            switch (type) {
                case 'BIAO_ZHUN_KUAI_DI':
                    html = html.replace(new RegExp("快递项目", "g"), '标准快递');
                    break;
                case 'SHUN_FENG_TE_HUI':
                    html = html.replace(new RegExp("快递项目", "g"), '顺丰特惠');
                    break;
                case 'DIAN_SHANG_TE_HUI':
                    html = html.replace(new RegExp("快递项目", "g"), '电商特惠');
                    break;
                case 'DIAN_SHANG_SU_PEI':
                    html = html.replace(new RegExp("快递项目", "g"), '电商速配');
                    break;
            }

        }
        if (html.indexOf("总件数") > 0) {
            html = html.replace(new RegExp("总件数", "g"), data.goodsCount);
        }
        if (html.indexOf("订单编号") > 0) {
            html = html.replace(new RegExp("订单编号", "g"), data.orderNo);
        }
        if (html.indexOf("条形码") > 0) {
            html = html.replace(new RegExp("条形码", "g"), data.shippingNo);
        }
        ...
        if (html.indexOf("发货时间") > 0) {
            ...
            if (data.deliveryTime) {
                ...
                deliveryTime = Ext.Date.format(deliveryTime, 'Y-m-d')
            }
        ...
        if (html.indexOf("商品编码及数量") > 0) {
            ...
            var data = [];
            Ext.each(prodetail, function (prodetail,i) {
                //data.push(prodetail.productNo + ':(' + prodetail.buyCount + ')' + "\\n");
                if(i%2 == 1){
                    data.push(prodetail.productNo +' '+ prodetail.cateName +' '+ prodetail.productSku + ' :(' + prodetail.buyCount + ')'+ "\\n");
                }else{
                    data.push(prodetail.productNo +' '+ prodetail.cateName +' '+ prodetail.productSku + ' :(' + prodetail.buyCount + ')');
                }
