...
Ext.define('Supplier.controller.Order', {
    ...
    views: ['Order', 'Search', 'List', 'Item', 'Batch','ItemTab', 'ExamineList', 'OrderLog'],
        ...
        this.control({
            ...
            "#item actioncolumn#goodRemove": {
                click: function (button, rowIndex, colIndex, item, e, selected) {
                    var itemStore = Ext.getCmp('orderItem').getStore(),root = this;
...
//                                itemStore.remove(selected);
                                Ext.Ajax.request({
                                    ...
                                    success: function (response, options) {
                                        ...
                                        if (data.success) {
                                            ...
                                            root.getOrderGood(data.msg);

                                            Espide.Common.tipMsg('订单项删除操作成功', '订单项删除操作成功');
                                        }
                                    },
            ...
            "#splitOrderBtn": {
                click: function () {
                    ...
                    var listOrderItemIds = com.getGridSels('OrderList','orderItemIds');
                    ...
                    var itemIds = com.getGridSelsId('orderItem');


                    console.log(listOrderItemIds[0]);
                    console.log(itemIds);
                    function checkHasItem(itemIds) {
                        var flag = true;

                        Ext.each(itemIds, function (item, key) {
                            //判断是否在京东或者天猫平台  才能嘻唰唰
                            if (!Ext.Array.contains(listOrderItemIds[0],item)) {
                                flag = false;
                            }
                        });
                        return flag;
                    ...
                    //判断是否选错了订单项
                    if(!checkHasItem(itemIds)){
                        com.showGridSelErr('警告，你选择的订单，不包含你选择的订单项！');
                        return;
                    }
            ...
            "#addGood": {
                click: function () {
                    ...
                    Ext.require("Supplier.controller.AddGoodWin", function () {
                        var app = Supplier.getApplication(), addController = app.getController('AddGoodWin');
                        var com = Espide.Common,
                        ...
                        if (!self.checkOrderState('OrderList')) return;
...
//
            ...
            "#addOrder": {
                click: function () {
                    ...
                    Ext.require("Supplier.controller.AddOrderWin", function () {

                        var app = Supplier.getApplication(), addOrderController = app.getController('AddOrderWin');
                        if (Ext.getCmp('addOrderWin')) return;
                        Ext.widget('addOrderWin').show(function () {
...
//                        }
            ...
            '#addOrderChange': {
                click: function () {
                    ...
                    Ext.require("Supplier.controller.AddOrderChangeWin", function () {


                        var app = Supplier.getApplication(), addOrderChangeController = app.getController('AddOrderChangeWin');

                        if (Ext.getCmp('addOrderChange')) return;
                        Ext.widget('addOrderChange').show(root,function () {
            ...
            '#exchangeOnsale': {
                click: function (btn) {
                    ...
                    var OrderListSlc = Ext.getCmp('OrderList').getSelectionModel().getSelection(),root = this;
                    ...
                    //显示售前换货窗口显示
                    Ext.require("Supplier.controller.ExchangeOnsaleWin", function () {

                        var app = Supplier.getApplication(), ExchangeController = app.getController('ExchangeOnsaleWin');
                        Ext.widget('exchangeOnsale').show(root,function () {

                            var ExchangeAddListStore = Ext.getCmp('ExchangeAddList').getStore();
                            var ExchangeItemStore = Ext.getCmp('ExchangeItem').getStore();
                            var ExchangeOnsaleCartStore = Ext.getCmp('ExchangeOnsaleCart').getStore();
                            ...
                            //清空上一次数据
                            if (ExchangeOnsaleCartStore.count() >= 0 ) {
                                ExchangeOnsaleCartStore.removeAll();
...
////                                records[0].data['platformType'] = records[0].data.outPlatformType.name;
////
////                                Ext.getCmp('ExchangeInfo').getForm().loadRecord(records[0]);
//                                Ext.getCmp('keyword').setValue(records[0].get('platformOrderNo'));
//
//                                //联动店铺数据
////                                var shopId = Ext.getCmp("shopGo");
...
//                            }
            ...
            '#replenishment': {
                click: function () {
                    var root = this;
                    //显示售前换货窗口显示
                    Ext.require("Supplier.controller.ReplenishmentWin", function () {

                        var app = Supplier.getApplication(), ReplenishmentController = app.getController('ReplenishmentWin');
                        if (Ext.getCmp('ReplenishmentWin')) return;
                        Ext.widget('ReplenishmentWin').show(root,function () {
